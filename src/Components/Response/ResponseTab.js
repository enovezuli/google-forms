...
function ResponseTab (props) {
  ...
  }, [props.formId, props.formData, setFormData])

  function getSelectedOption (qId, i, j) {
    const oneResData = responseData[j]
    // console.log(oneResData);

    const selectedOp = oneResData.response.filter(qss => qss.questionId === qId)
    console.log(selectedOp)

    if (selectedOp.length > 0) {
      const finalOption = questions[i].options.find(oo => oo._id === selectedOp[0].optionId)
